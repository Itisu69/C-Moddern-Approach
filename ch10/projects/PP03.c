// Removing num_in_rank, num_in_suit, and card_exists arrays from the poker.c
// Program will store the cards in the 5 x 2 array instead.
//
// Each row will represent a card.For example, if the array is named hand, then
// hand[0][0] will store the rank of the first card and hand[0][1] will store
...
void read_cards(void) {
    ...
    while (card_read < NUM_CARDS) {
        ...
        else if (duplicate_card(rank, suit, hand, card_read))
...
/* duplicate_card : determines if given card has already been read */
bool duplicate_card(int rank, int suit, int hand[NUM_CARDS][2], int card_read) {
...
/*analayze_hand: Determines whether the hand cotains a straight, flush,
four-of-a-kind, and/or three-of-a-kind; determines the number of pairs; stores
the results into the external variables straight, flush, four, three, and
pairs.*/

void analyze_hand(void) {
    int num_consec = 0;
    int card, matches, rank;
